...
template<typename T>
PsfXml<T>::PsfXml() {
    stringToModelMap_["GibsonLani"] = Psf<T>::MODEL_GIBSON_LANI;
    stringToModelMap_["Haeberle"] = Psf<T>::MODEL_HAEBERLE;
    modelToStringMap_[Psf<T>::MODEL_GIBSON_LANI] = "GibsonLani"; 
...
bool PsfXml<T>::open( 
    ...
    if ( confocal ) {
        this->fsize_ = extractValue(psfElement->FirstChildElement("Size"), "value");
		this->distance_ = extractValue(psfElement->FirstChildElement("Distance"), "value");
		this->magY_ = extractValue(psfElement->FirstChildElement("Magnification"), "value");
    }
...
bool PsfXml<T>::save(
    ...
    comment->SetValue(" PSF specification file ");
    doc.LinkEndChild( comment );

    comment = new TiXmlComment();
    comment->SetValue("     model = <\"GibsonLani\" | \"Haeberle\" > " );
    doc.LinkEndChild( comment );
