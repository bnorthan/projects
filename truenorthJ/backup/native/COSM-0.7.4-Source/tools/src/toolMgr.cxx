...
ToolMgr::convolve( 
    ...
    {
        blitz::Array<float, 3> res = convolutionWithFFT(img1.GetFloatArray(),img2.GetFloatArray());
        return wuDataWrite(res, outName_) == 0;
    }
    else if ( img1.IsDouble() && img2.IsDouble() )
    {
        blitz::Array<double, 3> res = convolutionWithFFT(img1.GetDoubleArray(),img2.GetDoubleArray());
        return wuDataWrite(res, outName_) == 0;
    }
    else if ( img1.IsLongDouble() && img2.IsLongDouble() )
    {
        blitz::Array<long double, 3> res = convolutionWithFFT(img1.GetLongDoubleArray(),img2.GetLongDoubleArray());
        return wuDataWrite(res, outName_) == 0;
    }
    return false;
}
...
ToolMgr::variant(
    ...
    {
        blitz::Array<float, 3> res = variantImage<float,3>(number, start, size);
        wuDataWrite(res, outName_);
        return true;
    }
    else if ( object.IsDouble() && psf.IsDouble() )
    {
        blitz::Array<double, 3> res = variantImage<double,3>(number, start, size);
        wuDataWrite(res, outName_);
        return true;
    }
    else if ( object.IsLongDouble() && psf.IsLongDouble() )
    {
        blitz::Array<long double, 3> res = variantImage<long double, 3>(number, start, size);
        wuDataWrite(res, outName_);
        return true;
    }
    return false;
