...
template<typename T, int N>
class GraphObserver : public cosm::EstimateObserver<T,N> {
  public:
    GraphObserver(
        unsigned period, 
        cosm::ErrorType type
    ) : cosm::EstimateObserver<T,N>(period), line_(0), type_(type), hasPhantom_(false) { };

    virtual ~GraphObserver() { };

    void setPhantom( blitz::Array<T,N>& phan ) 
    { 
        phantom_.resize(phan.extent()); 
        phantom_ = phan; 
        hasPhantom_ = true; 
    }; 
  ...
  protected:
    ...
    {
        T error =  errorEstimate_.error( type_, estimate.results(),
            hasPhantom_ ? phantom_ : estimate.getOldEstimate() );

        // update graph
        double x = (double) estimate.getIterationsDone();
        line_ = new Ca_LinePoint(line_, x, double(error), 0, FL_BLACK);
        Fl::wait(0.0);
    };

private:
    Ca_LinePoint* line_;
    cosm::ErrorType type_;
    bool hasPhantom_;
    blitz::Array<T, N> phantom_;
    cosm::ErrorEstimate<T,N> errorEstimate_;
};
...
blitz::Array<T,N> performEstimation(
    ...
    observer = new ErrorObserver<T,N>(update, (cosm::ErrorType)err);
    if ( usePhantom )
    {
    ...
    if ( prog != NULL )
    {
        ...
        observers.push_back(observer);
        observer = new GraphObserver<T,N>(update, (cosm::ErrorType)err);
        if ( usePhantom )
        {
            ((GraphObserver<T,N>*)observer)->setPhantom(phantom);
        }
        observers.push_back(observer);
