...
public class DeconvolutionCommand<T extends RealType<T> & NativeType<T>> extends UmbrellaCommand
{
	...
	public void run()
	{
	    ...
	    Dataset extended = (Dataset)commandModule.getOutputs().get("output");
		
		String psfCommandName=psfModel.getCommandClassName();
	    Module module=psfModel.getModule();
	    
	    Iterator it = psfModel.getModule().getInputs().entrySet().iterator();
	    
	    Map inputMap=psfModel.getModule().getInputs();
	    
	    Dataset psf;
	
	    // if non circulent model
	    if (isNonCirculent())
	    {
		    ...
		    commandModule= runAndBlock(psfCommandName, inputMap);
		    ...
		    psf = (Dataset)commandModule.getOutputs().get("output");
	    ...
	    // Debugging -- Pring out parameters of PSF commmand
